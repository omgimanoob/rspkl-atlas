...
async function main() {
  ...
  // 1) Basic counts
  try {
    const [rows]: any = await atlasPool.query('SELECT COUNT(*) AS total, SUM(kimai_project_id IS NULL) AS atlas_null FROM project_overrides')
    console.log('project_overrides counts:', rows?.[0])
  } catch (e: any) {
    console.error('ERROR: counting project_overrides failed:', e?.message || e)
  }

  // 2) Sample Prospective rows
  try {
    const [rows]: any = await atlasPool.query(
      'SELECT id, kimai_project_id, is_prospective, status_id, JSON_EXTRACT(extras_json, "$.name") AS name, created_at, updated_at FROM project_overrides WHERE kimai_project_id IS NULL ORDER BY id DESC LIMIT 5'
    )
