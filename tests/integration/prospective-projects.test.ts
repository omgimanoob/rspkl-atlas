...
describe('Prospective projects admin API', () => {
  ...
  it('creates and lists a prospective project', async () => {
    const create = await agent.post('/api/admin/prospective').send({ name: 'Prospect A', notes: 'tbd' }).expect(201);
    ...
    const list = await agent.get('/api/admin/prospective').expect(200);
  ...
  it('links a prospective project to a Kimai project id', async () => {
    ...
    const link = await agent.post(`/api/admin/prospective/${createdId}/link`).send({ kimai_project_id: kimaiId }).expect(200);
    ...
    await agent.post(`/api/admin/prospective/${createdId}/link`).send({ kimai_project_id: kimaiId }).expect(400);
  });
});
